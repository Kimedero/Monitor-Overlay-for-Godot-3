...
#	draw_string(font, Vector2(64, 64), "Hello world")
...
func _draw_graph() -> void:
	...
	var points = PoolVector2Array() # PackedVector2Array()
...
func remap(p_value: float, p_istart: float, p_istop: float, p_ostart: float, p_ostop: float):
	return lerp(p_ostart, p_ostop, inverse_lerp(p_istart, p_istop, p_value))


func snapped(p_value, p_step):
	if (p_step != 0):
